...
app.post("/account/api/oauth/token", async (req, res) => {
    ...
    switch (req.body.grant_type) {
        case "client_credentials":
            ...
            res.json({
                ...
                expires_in: Math.round(
                    ((DateAddHours(new Date(decodedClient.creation_date), decodedClient.hours_expire).getTime()) -
                        new Date().getTime()) /
                        1000
                ),
        ...
        case "password":
            ...
            if (!req.user) return err();
                ...
                if (!(await bcrypt.compare(String(password), String(req.user.password)))) return err();
            }
            break;
        ...
        case "refresh_token":
            ...
            try {
                ...
                if (
                    DateAddHours(new Date(decodedRefreshToken.creation_date), decodedRefreshToken.hours_expire).getTime() <=
                    new Date().getTime()
                ) {
   ...
   Token.UpdateTokens();
    ...
    res.json({
        ...
        expires_in: Math.round(
            ((DateAddHours(new Date(decodedAccess.creation_date), decodedAccess.hours_expire).getTime()) -
                new Date().getTime()) /
                1000
        ),
        ...
        refresh_expires: Math.round(
            ((DateAddHours(new Date(decodedRefresh.creation_date), decodedRefresh.hours_expire).getTime()) -
                new Date().getTime()) /
                1000
        ),
